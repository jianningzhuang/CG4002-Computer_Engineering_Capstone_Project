...
class FPGA:
    ...
    def process_IMU(self, IMU_data, player_num):
        try:
            if player_num == 1:
...
class GameEngine:
    ...
    def wait_for_actions(self):
        while (self.player_1_action == Actions.no or self.player_2_action == Actions.no):
            ...
            if self.player_1_action == Actions.no and not action_queue_player_one.empty():
                ...
                # game_state_queue_to_relay_laptop_one.put({"p1": self.player_1_action, "p2": Actions.no})

            if self.player_2_action == Actions.no and not action_queue_player_two.empty():
                self.player_2_action = action_queue_player_two.get()
                # game_state_queue_to_relay_laptop_two.put({"p1": Actions.no, "p2": self.player_2_action})
    ...
    def update_game_engine(self, updated_game_state):
        ...
        while not action_queue_player_one.empty():
            p1_leftover_action = action_queue_player_one.get()
            print('[Game Engine] Player 1 leftover action: ' + p1_leftover_action)
        while not action_queue_player_two.empty():
            p2_leftover_action = action_queue_player_two.get()
            print('[Game Engine] Player 2 leftover action: ' + p2_leftover_action)


class RelayLaptopServer(threading.Thread):
    def __init__(self, port_num, player_num, fpga):
        super().__init__()
        ...
        while not (global_shutdown.is_set() or self.shutdown.is_set()):
            ...
            try:
                ...
                if sensor_data['data_type'] == "IMU":
                    player_num = sensor_data['player_num']
                    IMU_data = sensor_data['data_value']
                    self.fpga.process_IMU(IMU_data, player_num)
    ...
    def run(self):
        while not (global_shutdown.is_set() or self.shutdown.is_set()):
            try:
                ...
                game_state_dict = self.game_engine.wait_for_actions()

                print("[Eval Client] Sending Game State: " +
                      str(game_state_dict))
                ...
                self.game_engine.update_game_engine(updated_game_state)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != _num_para:
        ...
        print('python3 ' + os.path.basename(__file__) +
              ' [Eval Server IP] [Eval Server Port] [Ultra96 Port 1] [Ultra96 Port 2] [Secret Key]')
    ...
    _eval_server_ip_address = sys.argv[1]
    _eval_server_port_num = int(sys.argv[2])

    _ultra96_port_one = int(sys.argv[3])
    _ultra96_port_two = int(sys.argv[4])

    _secret_key = sys.argv[5]

    fpga = FPGA()

    laptop_server_one = RelayLaptopServer(_ultra96_port_one, 1, fpga)
    laptop_server_two = RelayLaptopServer(_ultra96_port_two, 2, fpga)
