...
class FPGA(threading.Thread):
    def __init__(self):
        ...
        self.player_one_difference = 0
        self.player_one_prev = -200

        self.player_two_difference = 0
        self.player_two_prev = -200
        ...
        while not global_shutdown.is_set():
            try:
                if not IMU_queue_player_one.empty():
                    ...
                    if self.player_one_prev == -200:
                        fill_start_time = time.time()
                        print("start to fill up frame: ", fill_start_time)
                        self.player_one_prev = IMU_data[0]
                    else:
                        self.player_one_difference += abs(
                            IMU_data[0] - self.player_one_prev)
                        self.player_one_prev = IMU_data[0]
                    ...
                    if len(self.player_one_data) == self.frame_size:
                        ...
                        print("time to fill up to action size: ",
                              full_frame_time)
                        if self.player_one_difference > self.threshold:
                ...
                if not IMU_queue_player_two.empty():
                    ...
                    if self.player_two_prev == -200:
                        self.player_two_prev = IMU_data[0]
                    else:
                        self.player_two_difference += abs(
                            IMU_data[0] - self.player_two_prev)
                        self.player_two_prev = IMU_data[0]
                    self.player_two_data.extend(IMU_data)
                    if len(self.player_two_data) == self.frame_size:
                        if self.player_two_difference > self.threshold:
                            action_data = np.asarray(
    ...
    def run(self):
        ...
        while not (global_shutdown.is_set() or self.shutdown.is_set()):
            ...
            try:
                ...
                if sensor_data['T'] == "M":
                    if sensor_data['P'] == 1:
                        IMU_queue_player_one.put(sensor_data['V'])
                    else:
                        IMU_queue_player_two.put(sensor_data['V'])
                elif sensor_data['T'] == "G":
                    shoot_receive_time = time.time()
                    print("shoot received: ", shoot_receive_time)
                    if sensor_data['P'] == 1:
                        action_queue_player_one.put(Actions.shoot)
                    else:
                        action_queue_player_two.put(Actions.shoot)
                elif sensor_data['T'] == "V":
                    hit_receive_time = time.time()
                    print("hit received: ", hit_receive_time)
                    if sensor_data['P'] == 1:
                        VEST_queue_player_one.put(1)
                    else:
                        VEST_queue_player_two.put(1)
                else:
                    print('[Relay Laptop Server ' + str(self.player_num) +
