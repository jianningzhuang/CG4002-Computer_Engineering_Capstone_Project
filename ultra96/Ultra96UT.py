...
class FPGA(threading.Thread):
    def __init__(self):
        ...
        self.player_one_action_frame = []
        self.player_two_action_frame = []

        self.p1_yaw_prev = None
        self.p1_pitch_prev = None
        self.p1_roll_prev = None

        self.p1_yaw_avg = None
        self.p1_pitch_avg = None
        self.p1_roll_avg = None

        self.p2_yaw_prev = None
        self.p2_pitch_prev = None
        self.p2_roll_prev = None

        self.p2_yaw_avg = None
        self.p2_pitch_avg = None
        self.p2_roll_avg = None
            ...
            try:
                if not IMU_queue_player_one.empty():
                    ...
                    if self.p1_start_of_move == True:
                        self.player_one_action_frame.extend(IMU_data)
                        if len(self.player_one_action_frame) == self.action_size:
                            action_data = np.asarray(
                                self.player_one_action_frame, dtype=np.float32)
                            ...
                            self.player_one_action_frame.pop(0)
                            self.player_one_action_frame.pop(0)
                            self.player_one_action_frame.pop(0)

                    self.player_one_data.extend(IMU_data)
                    if len(self.player_one_data) == self.frame_size*3:
                        print("[FPGA] Process Average")
                        self.p1_yaw_avg = sum(
                            self.player_one_data[0::3])/self.frame_size
                        self.p1_pitch_avg = sum(
                            self.player_one_data[1::3])/self.frame_size
                        self.p1_roll_avg = sum(
                            self.player_one_data[2::3])/self.frame_size
                        if self.p1_yaw_prev != None:
                            avg_check = [abs(self.p1_yaw_avg - self.p1_yaw_prev) > self.threshold, abs(self.p1_pitch_avg -
                                                                                                       self.p1_pitch_prev) > self.threshold, abs(self.p1_roll_avg - self.p1_roll_prev) > self.threshold]
                            if sum(avg_check) >= 1:
                                print("[FPGA] Player 1 threhold exceeded")
                                self.p1_start_of_move = not self.p1_start_of_move
                                self.player_one_action_frame = []

                        self.p1_yaw_prev = self.p1_yaw_avg
                        self.p1_pitch_prev = self.p1_pitch_avg
                        self.p1_roll_prev = self.p1_pitch_avg
                        self.player_one_data.pop(0)
                        self.player_one_data.pop(0)
                        self.player_one_data.pop(0)
                ...
                if not IMU_queue_player_two.empty():
                    ...
                    if self.p2_start_of_move == True:
                        self.player_two_action_frame.extend(IMU_data)
                        if len(self.player_two_action_frame) == self.action_size:
                            action_data = np.asarray(
                                self.player_two_action_frame, dtype=np.float32)
                            predicted_action = self.labels[np.argmax(
                                predict(action_data, self.ol, self.dma))]
                            ...
                            self.player_two_action_frame.pop(0)
                            self.player_two_action_frame.pop(0)
                            self.player_two_action_frame.pop(0)

                    self.player_two_data.extend(IMU_data)
                    if len(self.player_two_data) == self.frame_size*3:
                        self.p2_yaw_avg = sum(
                            self.player_two_data[0::3])/self.frame_size
                        self.p2_pitch_avg = sum(
                            self.player_two_data[1::3])/self.frame_size
                        self.p2_roll_avg = sum(
                            self.player_two_data[2::3])/self.frame_size
                        if self.p2_yaw_prev != None:
                            avg_check = [abs(self.p2_yaw_avg - self.p2_yaw_prev) > self.threshold, abs(self.p2_pitch_avg -
                                                                                                       self.p2_pitch_prev) > self.threshold, abs(self.p2_roll_avg - self.p2_roll_prev) > self.threshold]
                            if sum(avg_check) >= 1:
                                print("[FPGA] Player 2 threhold exceeded")
                                self.p2_start_of_move = not self.p2_start_of_move
                                self.player_two_action_frame = []

                        self.p2_yaw_prev = self.p2_yaw_avg
                        self.p2_pitch_prev = self.p2_pitch_avg
                        self.p2_roll_prev = self.p2_pitch_avg
                        self.player_two_data.pop(0)
                        self.player_two_data.pop(0)
                        self.player_two_data.pop(0)
...
class GameEngine(threading.Thread):
    ...
    def commit_action(self, pos_p1, pos_p2):
        ...
        print('[Game Engine] P2 IMU Queue Cleared')
...
class RelayLaptopServer(threading.Thread):
    ...
    def run(self):
        ...
        recv_thread = threading.Thread(target=self.recv_sensor_data)
        recv_thread.start()
...
class EvalClient(threading.Thread):
    ...
    def run(self):
        ...
        while not (global_shutdown.is_set() or self.shutdown.is_set()):
            try:
                if not game_state_queue_to_eval_server.empty():
