...
class StateStaff(PlayerStateBase):
    # update the player statistics after both player actions happened
    def update(self, pos_self, pos_opponent, action_self, action_opponent, action_opponent_is_valid):
        ...
        if self.shield_time > 0:
            # at least 10-(>10) = -ve if shield started more than 10s ago
            self.shield_time = max(
                self.shield_max_time-(time.time()-self.shield_start_time), 0)

        # check if shield is active
        if self.shield_time > 0:  # check shield time again after update
        ...
        if action_opponent_is_valid:
            if pos_self == 4 and pos_opponent != 4 and action_opponent == Actions.shoot:
                # we are protected from harm
                pass
            elif pos_self != 4 and pos_opponent == 4 and action_opponent == Actions.shoot:
                # we are protected from harm
                pass
            else:
                hp_reduction = 0
