...
class NotificationDelegate(DefaultDelegate):
    ...
    def process_IR_data(self, full_packet):
        try:
            ...
            if full_packet[0] == 71:  # GUN
                if not (SINGLE_PLAYER_MODE == True and PLAYER == 2):  # only send P1 GUN data to queue
                    gun_data_dict = dict()
            ...
            if full_packet[0] == 86:  # VEST
                if not (SINGLE_PLAYER_MODE == True and PLAYER == 2):  # only send P1 VEST data to queue
                    vest_data_dict = dict()
    ...
    def process_motion_data(self, full_packet):
        try:
            ...
            # push sensor data dict onto sensor_data_queue
            if not (SINGLE_PLAYER_MODE == True and PLAYER == 2):  # only send P1 MOTION data to queue
                ...
                if self.peripheral_obj_beetle.waitForNotifications(2.0):
                    ...
                    if ACK_IR_SENT_flag[self.peripheral_obj_beetle.addr]:
                        ...
                        if game_state_queue_to_beetle.empty():  # ACK packet to reply to GUN or VEST packet
                            ...
                            packet4 = struct.pack(packetFormat, statusByte, p1_hp, p1_bullets, p1_shield_time, p1_shield_health, *padding)
