...
class NotificationDelegate(DefaultDelegate):
    ...
    def process_motion_data(self, full_packet):
        try:
            ...
            # row = [float_y, float_p, float_r, str(now1)]
            # print(row)
        ...
        try:
            while True:
                ...
                if self.peripheral_obj_beetle.waitForNotifications(1.0):
                    ...
                    #         packet4 = struct.pack(packetFormat, statusByte, p1_hp, p1_shield_time, p1_shield_health, p1_bullets, *padding)
                    #         self.serial_chars.write(packet4, withResponse=False)
                    #         ACK_IR_SENT_flag[self.peripheral_obj_beetle.addr] = False
...
class RelayLaptopClient(threading.Thread):
    ...
    def add_timeout_data_dict(self):
        ...
        timeout_data_dict['data_value'] = 1
        sensor_data_queue_from_beetle.put(timeout_data_dict)
        print("[Relay Laptop Client] Send Timeout to get update")
...
if __name__ == '__main__':
    ...
    if _player_num == 1:
        # without IMU
        '''
        BTL_ADDRLIST = [BTL_GUN]
        BTL_TYPE = {BTL_GUN: 'G'}
        BTL_RESET_STATUS = {BTL_GUN: False}
        BTL_HANDSHAKE_STATUS = {BTL_GUN: False}
        ACK_HANDSHAKE_SENT_flag = {BTL_GUN: False}
        ACK_IR_SENT_flag = {BTL_GUN: False}
        sequence_number = {BTL_GUN: 0}
        prev_sequence_number = {BTL_GUN: 0}
        COUNT_GOOD_PKT = {BTL_GUN: 0}
        COUNT_FRAG_PKT = {BTL_GUN: 0}
        COUNT_DROPPED_PKT = {BTL_GUN: 0}
        '''

        # without VEST
        '''
        BTL_ADDRLIST = [BTL_GUN, BTL_IMU]
        BTL_TYPE = {BTL_GUN: 'G', BTL_IMU: 'M'}
        BTL_RESET_STATUS = {BTL_GUN: False, BTL_IMU: False}
        BTL_HANDSHAKE_STATUS = {BTL_GUN: False, BTL_IMU: False}
        ACK_HANDSHAKE_SENT_flag = {BTL_GUN: False, BTL_IMU: False}
        ACK_IR_SENT_flag = {BTL_GUN: False, BTL_IMU: False}
        sequence_number = {BTL_GUN: 0, BTL_IMU: 0}
        prev_sequence_number = {BTL_GUN: 0, BTL_IMU: 0}
        COUNT_GOOD_PKT = {BTL_GUN: 0, BTL_IMU: 0}
        COUNT_FRAG_PKT = {BTL_GUN: 0, BTL_IMU: 0}
        COUNT_DROPPED_PKT = {BTL_GUN: 0, BTL_IMU: 0}
        '''
